        ...
        <div className="p-6">
            ...
            <div className="bg-[#1a1a1a] text-white rounded-2xl p-5 mb-6">
                ...
                <button
                    onClick={async () => {
                        if (!current) return
                        const blob = await pdf(<PdfDocument assignment={current} />).toBlob()
                        const url = URL.createObjectURL(blob)
                        const a = document.createElement('a')
                        a.href = url
                        a.download = `${current.title.replace(/\s+/g, '_')}_QuestionPaper.pdf`
                        a.click()
                        URL.revokeObjectURL(url)
                    }}
                    className="mt-3 flex items-center gap-2 bg-white text-gray-900 px-4 py-2 rounded-full text-sm font-medium hover:bg-gray-100 transition-colors"
                >
                    <span>📄</span>
                    Download as PDF
                </button>
